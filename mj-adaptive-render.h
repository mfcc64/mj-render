                         ...
                         int max_iter, int is_julia)
{
    ...
    if (width < height) {
        ...
        T zy = (center_y - middle_y) * pixel_width;
        for (int x = left_x + 1; x <= right_x - 1; x++) {
            T zx = (x - center_x) * pixel_width;
            surface(x, middle_y) = mj_calc_select(cx, cy, zx, zy, max_iter, is_julia);
        }

        mj_recursive_render(surface, cx, cy, center_x, center_y, pixel_width,
                            left_x, right_x, top_y, middle_y, max_iter, is_julia);
        mj_recursive_render(surface, cx, cy, center_x, center_y, pixel_width,
                            left_x, right_x, middle_y, bottom_y, max_iter, is_julia);
    } else {
        int middle_x = (left_x + right_x) / 2;
        T zx = (middle_x - center_x) * pixel_width;
        for (int y = top_y + 1; y <= bottom_y - 1; y++) {
            T zy = (center_y - y) * pixel_width;
            surface(middle_x, y) = mj_calc_select(cx, cy, zx, zy, max_iter, is_julia);
        }

        mj_recursive_render(surface, cx, cy, center_x, center_y, pixel_width,
                            left_x, middle_x, top_y, bottom_y, max_iter, is_julia);
        mj_recursive_render(surface, cx, cy, center_x, center_y, pixel_width,
                            middle_x, right_x, top_y, bottom_y, max_iter, is_julia);
    }
}
                        ...
                        int max_iter, int is_julia)
{
    for (int x = 0; x < surface.width(); x++) {
        T zx = (x - center_x) * pixel_width;
        T zy = center_y * pixel_width;
        surface(x,0) = mj_calc_select(cx, cy, zx, zy, max_iter, is_julia);

        int y = surface.height() - 1;
        zy = (center_y - y) * pixel_width;
        surface(x,y) = mj_calc_select(cx, cy, zx, zy, max_iter, is_julia);
    }

    for (int y = 1; y < surface.height() - 1; y++) {
        T zx = -center_x * pixel_width;
        T zy = (center_y - y) * pixel_width;
        surface(0,y) = mj_calc_select(cx, cy, zx, zy, max_iter, is_julia);
